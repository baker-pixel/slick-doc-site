...
import { Loader2, Rocket, CheckCircle, Circle, Play, Mail, Calendar, Star, BarChart3, RefreshCw, Lock, Zap } from "lucide-react";
...
export function OnboardingAutomationPanel({ adminPassword }: { adminPassword?: string }) {
  ...
  const fetchAll = async () => {
    setLoading(true);
    const [onboardingRes, workflowRes] = await Promise.all([
      supabase
        .from("client_onboarding")
        .select(`*, client_accounts(business_name, email, tier, first_name)`)
    ...
    if (onboardingRes.error) {
      ...
      setOnboardings((onboardingRes.data || []) as ClientOnboarding[]);
    }

    if (!workflowRes.error && workflowRes.data) {
      // Load steps for each workflow
      const wfIds = workflowRes.data.map((w: any) => w.id);
      ...
      const enriched: ClientWorkflow[] = workflowRes.data.map((w: any) => ({
        ...w,
        steps: stepsMap.get(w.id) || [],
      }));
  ...
  const fetchOnboardings = fetchAll;
        ...
        {loading ? (
          ...
          <Tabs defaultValue="workflows">
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="onboarding">Admin Checklist</TabsTrigger>
            </TabsList>

            {/* ── Tab 1: Workflow Steps (authoritative) ── */}
            ...
            {/* ── Tab 2: Admin checklist (legacy client_onboarding) ── */}
