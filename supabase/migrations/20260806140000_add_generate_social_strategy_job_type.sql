-- New onboarding step (seed-tier-workflow FOUNDATION_STEPS) generates the
-- client's social content strategy immediately on day one, same treatment
-- as the SEO audit -- previously the only path to this was the weekly
-- client-context-refresh cron, so a new client's Social agent could sit at
-- "Setting Up" for up to a week. automation_jobs.job_type's check
-- constraint predates this and needs the new value added explicitly.
